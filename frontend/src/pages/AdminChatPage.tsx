...
function StatCard({ label, value, Icon, accent }: { label: string; value?: number; Icon: typeof Users; accent?: boolean }) {
  return (
    <div className={`bg-dark-card border rounded-lg p-3 ${accent ? 'border-dark-gold/30' : 'border-dark-border'}`}>
      ...
      <div className={`text-xl font-semibold ${accent ? 'text-dark-gold' : 'text-[#d4c4a0]'}`}>{value ?? '—'}</div>
    </div>
  )
}
...
export default function AdminChatPage() {
  ...
  return (
    <div className={`space-y-5 ${contentWidth}`}>
      <button type="button" onClick={goBack} className="text-[#8a7a60] hover:text-[#d4c4a0] text-sm flex items-center gap-1 transition-colors">
      ...
      <div>
        ...
        <p className="text-xs text-[#8a7a60] mt-1">{t('chatAdmin.subtitle')}</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-dark-card border border-dark-border rounded-lg p-3">
          ...
          <div className="text-sm font-semibold text-[#d4c4a0] truncate">
      ...
      <div className="bg-dark-card border border-dark-border rounded-lg p-4 space-y-2.5">
        <div className="flex items-center gap-2 text-sm font-medium text-[#d4c4a0]">
        ...
        {announcement?.message && (
          <div className="flex items-start justify-between gap-2 rounded-lg bg-dark-gold/10 border border-dark-gold/25 px-3 py-2">
            <p className="text-xs text-[#d4c4a0] flex-1">{announcement.message}</p>
            <button
        ...
        <div className="flex items-end gap-2">
          <textarea
            ...
            className="flex-1 resize-none bg-dark-bg border border-dark-border rounded-lg px-3 py-2 text-sm text-[#d4c4a0]
              placeholder:text-[#8a7a60]/50 focus:outline-none focus:border-dark-gold/40"
          />
          <button
            ...
            <div key={m.id} className="flex items-start gap-2.5 bg-dark-card border border-dark-border rounded-lg px-3 py-2 group">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  <span className="text-xs font-medium text-[#d4c4a0]">{m.username}</span>
                  {m.role === 'ADMIN' && <Shield className="w-2.5 h-2.5 text-dark-gold" />}
                  {m.role === 'SUPER_ADMIN' && <Crown className="w-2.5 h-2.5 text-dark-gold" />}
                  <span className="text-[10px] text-[#8a7a60]/70">{new Date(m.createdAt).toLocaleString()}</span>
                </div>
                <p className="text-sm text-[#d4c4a0] break-words whitespace-pre-wrap mt-0.5">{m.content}</p>
              </div>
              <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                <button
                  onClick={() => setMuteTarget({ id: m.userId, username: m.username })}
                  title={t('chatAdmin.mute.action')}
                  className="p-1.5 rounded border border-dark-border text-[#8a7a60] hover:text-amber-400 hover:border-amber-400/40 transition-colors"
                >
                  <MicOff size={13} />
                </button>
                <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setMuteTarget(null)}>
          <div className="bg-dark-card border border-dark-border rounded-xl w-full max-w-sm p-5 space-y-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setMuteTarget(null)} className="text-[#8a7a60] hover:text-[#d4c4a0]"><X size={16} /></button>
            </div>
            <p className="text-xs text-[#8a7a60]">{t('chatAdmin.mute.forUser', { name: muteTarget.username })}</p>
            <div className="grid grid-cols-3 gap-2">
              <button onClick={() => doMute(60)} className="py-2 rounded-lg text-xs bg-dark-bg border border-dark-border text-[#d4c4a0] hover:border-dark-gold/40 transition-colors">{t('chatAdmin.mute.hour')}</button>
              <button onClick={() => doMute(1440)} className="py-2 rounded-lg text-xs bg-dark-bg border border-dark-border text-[#d4c4a0] hover:border-dark-gold/40 transition-colors">{t('chatAdmin.mute.day')}</button>
            ...
            <button
              ...
              className="w-full py-2 rounded-lg text-xs text-[#8a7a60] border border-dark-border hover:text-[#d4c4a0] transition-colors"
            >
