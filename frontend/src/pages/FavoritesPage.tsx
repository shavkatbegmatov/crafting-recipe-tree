  ...
  if (!user) {
    return (
      <div className={`${contentWidth} text-center py-16`}>
        <Star size={32} className="mx-auto text-skin-dark mb-3" />
        <p className="text-sm text-skin-muted mb-4">{t('favorites.loginRequired')}</p>
        <Link
          to="/login"
  ...
  return (
    <div className={`space-y-5 ${contentWidth}`}>
      ...
      <div>
        ...
        <p className="text-xs text-skin-muted mt-1">{t('favorites.subtitle')}</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="py-16 text-center">
          <Star size={32} className="mx-auto text-skin-dark mb-3" />
          <p className="text-sm text-skin-muted">{t('favorites.empty')}</p>
          <p className="text-xs text-skin-dark mt-1">{t('favorites.emptyHint')}</p>
        </div>
      ) : (
