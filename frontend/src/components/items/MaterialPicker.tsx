...
export default function MaterialPicker({ selected, onChange, searchPlaceholder, emptyHint, targetsLabel }: Props) {
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Tanlangan itemlar */}
      {selected.length === 0 ? (
        <div className="bg-dark-card border border-dark-border rounded-lg py-8 text-center text-sm text-[#5a4e3a]">
          {emptyHint}
        </div>
      ) : (
        <div className="space-y-2">
          <span className="text-[10px] font-medium text-[#8a7a60] uppercase tracking-wider">{targetsLabel}</span>
          {selected.map((s) => {
