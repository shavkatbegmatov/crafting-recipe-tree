  ...
  return (
    <div className="bg-dark-card border border-dark-border rounded-lg p-5">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-dark-border">
              <th className="text-left py-2 px-3 text-[#8a7a60] font-medium">Xomashyo</th>
              <th className="text-right py-2 px-3 text-[#8a7a60] font-medium">Miqdor</th>
            </tr>
          </thead>
          <tbody>
            {data.rawMaterials.map((mat) => (
              <tr key={mat.name} className="border-b border-dark-border/50 hover:bg-dark-hover transition-colors">
                <td className="py-2 px-3 text-[#d4c4a0]">{mat.name}</td>
      ...
      <div className="mt-4 pt-3 border-t border-dark-border flex items-center gap-2 text-sm">
        ...
        <span className="text-[#8a7a60]">Jami kraft vaqti (ketma-ket):</span>
